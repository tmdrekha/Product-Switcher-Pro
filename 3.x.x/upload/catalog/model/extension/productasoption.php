...
class ModelExtensionProductasoption extends Model {
    public function getProductSwitchOptions($product_id) {
		...
		$main_product_id=0;
		$query=$this->db->query("SELECT * FROM `" . DB_PREFIX . "productasoption_product` WHERE product_id = '" . (int)$product_id . "'");
		...
		if(empty($main_product_id)){
			$query=$this->db->query("SELECT main_product_id FROM `" . DB_PREFIX . "productasoption` WHERE main_product_id = '" . (int)$product_id . "' AND status='1'");
			if(!empty($query->row['main_product_id']))
			{
				$main_product_id=$query->row['main_product_id'];
			}
		...
		if(!empty($main_product_id)){
			$product_option_data=[];
			$query=$this->db->query("SELECT * FROM `" . DB_PREFIX . "productasoption` WHERE main_product_id = '" . (int)$main_product_id . "' AND status='1'");
			...
			$queryoptions=$this->db->query("SELECT * FROM `" . DB_PREFIX . "productasoption_product` WHERE switch_id = '" . (int)$switch_id . "' order by sort_order");
